  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      cell.addEventListener('touchstart', (e) => {
        pressTimer = setTimeout(() => onCellRightClick(e), 600);
      });
...
function checkWin() {
  ...
  if (revealedCount >= totalSafe) {
    ...
    messageEl.textContent = 'You cleared the board — you win!';
    const score = computeScore();
    saveLastScore(score, true);
  }
}
...
function computeScore() {
  ...
  const timeFactor = Math.max(1, 1 + (300 - timeElapsed) / 300);
  return Math.round(base * mult * timeFactor);
}

// ======= Leaderboard =======
function loadLocalLeaderboard() {
  const raw = localStorage.getItem('ms_highscores');
  return raw ? JSON.parse(raw) : [];
}

function saveLocalLeaderboard(list) {
  localStorage.setItem('ms_highscores', JSON.stringify(list));
}

function saveLastScore(score, won) {
...
function renderLocalLeaderboard() {
  const list = loadLocalLeaderboard();
  if (list.length === 0) {
    leaderboardEl.innerHTML = '<div style="color:var(--muted)">No local scores yet.</div>';
    return;
  }
